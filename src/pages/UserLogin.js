...
function UserLogin() {
  ...
  const handlesubmit = async (e) => {
    ...
    if (json.sucess) {
      localStorage.setItem("orsoot_user", json.authtoken);
        alert("logged in");
        navigate("/")
    } else {
      if (json.field) {
  ...
  return (
      ...
      <div className="auth_form">
        ...
        <div className="auth_form_submit userlogin">
          <button className="auth_submit_btn" onClick={() => {
            if (phone == 9076980101 && password == 123456) {
              navigate("/dashboard")
            }
            else {
          ...
          <div className="auth_login_btns">
            ...
            <button className=" loginbtn" onClick={()=>navigate("/")}>Register</button>
            <button className=" loginbtn" style={{flex:1}}>Forgot Password?</button>
...
export default UserLogin;
      ...
      //   </button>
      //   <br />
      //   dont have account <Link to={"/userregister"}>register</Link>
      // </form>